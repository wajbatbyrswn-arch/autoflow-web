...
export default function Complaints() {
  ...
  async function load() {
    ...
    try {
      ...
      setItems(data)
    } catch { toast.error(t('تعذّر جلب الشكاوى')) }
    setLoading(false)
  }
  useEffect(() => { load() }, [])
  ...
  async function markResolved(c) {
    ...
    toast.success(t('تم تعليمها كمحلولة'))
  }
  async function resumeAI(c) {
    if (!c.conversation_id) return
    await window.api?.conversations?.resumeAI?.(c.conversation_id)
    toast.success(t('تم استئناف الرد الذكي'))
  }
  async function del(c) {
    const ok = await confirm({ title: t('حذف الشكوى'), message: t('هل تريد حذف هذه الشكوى من السجل نهائياً؟'), confirmText: t('حذف'), dangerous: true, rememberKey: 'complaint_delete' })
    if (!ok) return
        ...
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
          {c.conversation_id && (
            <button onClick={() => nav(`/conversations?open=${c.conversation_id}`)}
              ...
              <MessageCircle size={14}/> {t('الانتقال إلى الدردشة')}
            </button>
          )}
          {!c.is_read && (
            <button onClick={() => markResolved(c)}
              ...
              <CheckCircle2 size={14}/> {t('تعليمها محلولة')}
            </button>
          )}
          {c.conversation_id && (
            <button onClick={() => resumeAI(c)}
              ...
              <Play size={14}/> {t('استئناف الرد الذكي')}
            </button>
          )}
          <button onClick={() => del(c)} title={t('حذف')}
  ...
  return (
    <div className="animate-fade" style={{ padding:24 }}>
      <div className="page-header">
        <h1 style={{ display:'flex', alignItems:'center', gap:10 }}><AlertOctagon size={26}/> {t('الشكاوى')}</h1>
        <p>{t('الشكاوى المرصودة تلقائياً من محادثات الزبائن. الرد الذكي يتوقف فوراً عند رصد شكوى.')}</p>
      </div>

      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, marginBottom:20 }}>
        <div className="card" style={{ padding:18, textAlign:'center' }}>
          <div style={{ fontSize:32, fontWeight:900, color:'#ef4444' }}>{open.length}</div>
          <div style={{ opacity:.6, fontSize:13 }}>{t('مفتوحة')}</div>
        </div>
        <div className="card" style={{ padding:18, textAlign:'center' }}>
          <div style={{ fontSize:32, fontWeight:900, color:'#10b981' }}>{resolved.length}</div>
          <div style={{ opacity:.6, fontSize:13 }}>{t('محلولة')}</div>
        </div>
        <div className="card" style={{ padding:18, textAlign:'center' }}>
          <div style={{ fontSize:32, fontWeight:900 }}>{items.length}</div>
          <div style={{ opacity:.6, fontSize:13 }}>{t('إجمالي')}</div>
        </div>
      </div>
      ...
      {!loading && items.length === 0 && (
        <div className="card" style={{ padding:40, textAlign:'center', opacity:.6 }}>
          ...
          <div>{t('لا توجد شكاوى — أداء ممتاز! 🎉')}</div>
        </div>
      )}

      {open.length > 0 && (
        <>
          <h3 style={{ margin:'10px 0 12px', fontSize:14, opacity:.7 }}>{t('الشكاوى المفتوحة')}</h3>
          {open.map(c => <Card key={c.id} c={c} />)}
        </>
      )}
      {resolved.length > 0 && (
        <>
          <h3 style={{ margin:'24px 0 12px', fontSize:14, opacity:.7 }}>{t('الشكاوى المحلولة')}</h3>
